 ...
 * @author Shai Almog
 */
public class DetailPropMineForm extends BaseForm {

    private Resources theme = UIManager.initFirstTheme("/theme");

    public DetailPropMineForm(Resources res, Propriete p1) {
        ...
        System.out.println(p1.getTypePropriete());
        typePropriete.setEnabled(true);
        ...
        add(BorderLayout.west(new Label("", "PaddedLabel")).
                add(BorderLayout.CENTER, update));
        update.requestFocus();
        update.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent evt) {
                Propriete p = new Propriete(p1.getId(),categorie.getText(),typePropriete.getText(), pays.getText(), ville.getText(), rue.getText(), titre.getText(), prix.getText(), nbrChambre.getText(), nbrVoyageur.getText(),description.getText());
                ProprieteCRUD pc = new ProprieteCRUD();
                pc.updatePropriete(p);
                Dialog.show("update avec succés", "Proprietée modifiée avec succés","ok",null);
               // new DetailPropMineForm(theme,p);
        ...
        delete.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent evt) {
                ProprieteCRUD pc = new ProprieteCRUD();
                 if(   Dialog.show("voulez vous supprimer cette proprietée", "", "ok", "annuler") ) {
                            pc.removePropriete(p1);
