...
public class ListReservationByTitre extends BaseForm {
    
    

    public ListReservationByTitre(Resources res) {
     ...
     popular.getAllStyles().setFont(largeBoldSystemFont);
      ...
      connectionRequest.setUrl("http://localhost/script/ListRservationByTitre.php");
        ...
        CrudRservation  fc = new CrudRservation ();
               ...
               for(Resrevation f : l){
                    ...
                    Container cnt = new Container();
                     Button b = new Button ();
                    Container cnt1 = addButton(res.getImage("log.png"),"                        "+ f.getTitre(),"Cette chambre est reserve "+f.getPrenom()+" fois",b,cnt,f.getId_r(),res);
   ...
   private Container  addButton(Image img, String title ,String et ,Button image,Container cnt,int id ,Resources res ) {
       ...
       image = new  Button(img.fill(width, height));
         image.addActionListener((evt) -> {
              Form form = Display.getInstance().getCurrent();
                      form.removeAll();
                       ListReservation n = new ListReservation(res,id);
                     n.show();
